...
server-side job applies and this pipeline cannot reach the scores for. The `analytic`
package docstring carries that. Its docstrings are the single copy of *why* each step is
shaped as it is; this section is the map, not a second copy. What a reader of the pipeline needs to know is that six of
