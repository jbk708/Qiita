...
`read` follows its own rule, checked before any file moves. A work ticket re-runs a step in a new attempt directory, so one ticket can register a prep_sample's reads a second time from a new staging dir. A prep_sample the same ticket registered before has its rows **replaced**; a prep_sample another ticket registered **refuses** the whole registration with `AlreadyExists`. `register_files` carries why, and why this takes no lock.
